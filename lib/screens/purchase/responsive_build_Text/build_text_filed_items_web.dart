...
class BuildTextFieldItemsWeb extends StatefulWidget {
  ...
  const BuildTextFieldItemsWeb(
      {super.key, required this.formControllers, required this.index, required this.focusNode});
...
class _BuildTextFieldItemsWebState extends State<BuildTextFieldItemsWeb> {
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      ...
                      onChanged: (value) {
                        setState(() {
                           itemIndex = provider.itemName.indexOf(provider.selectedItemName.toString());
                          log("Check Index:: ${itemIndex}");
                          if(widget.focusNode.hasFocus){
                            log("Enter");
                            // updateQuantityForIndex(itemIndex,isFocus: true);
                            // final value = provider.itemName[itemIndex];
                            updateFields(itemIndex, provider, widget.formControllers);
                          }else{
  ...
  void updateQuantityForIndex(index,{bool isFocus = false}) {
    double quantity =
        double.tryParse(widget.formControllers.quantityController.text) ?? 0;
    if(isFocus){
      if (index == itemIndex) {
  ...
  void updateFields(int? itemIndex,ItemsDataProvider _itemsDataProvider,FormControllers _formControllers) {
    if (itemIndex != null) {
      // int itemIndex = _itemsDataProvider.itemName.indexOf(value);
      _itemsDataProvider.selectedItemNameId = _itemsDataProvider.itemsID[itemIndex];
      _itemsDataProvider.selectedItemSalePrice = _itemsDataProvider.itemSalePrice[itemIndex];
      _itemsDataProvider.selectedItemPurchasePrice = _itemsDataProvider.itemPurchasePrice[itemIndex];
      _itemsDataProvider.selectedItemStock = _itemsDataProvider.itemStock[itemIndex];
      _itemsDataProvider.selectedUom = _itemsDataProvider.uom[itemIndex];

      // _formControllers.itemNameController.text = pro;
      _formControllers.itemCodeController.text = _itemsDataProvider.itemsID[itemIndex];
      _formControllers.uomController.text = _itemsDataProvider.uom[itemIndex];
      _formControllers.stockController.text = _itemsDataProvider.itemStock[itemIndex];
      _formControllers.saleRateController.text = _itemsDataProvider.itemSalePrice[itemIndex];
      _formControllers.priceRateController.text = _itemsDataProvider.itemPurchasePrice[itemIndex];
